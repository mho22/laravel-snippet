...
// /laravel-snippets/<path>/ so the GH Pages subdirectory deploy resolves.
function rewriteForGhPages(html: string, currentRoute: string): string {
    ...
    // bar to '/<route>' on mount, dropping '/laravel-snippets/'.
    ...
    html = html.replace(
        ...
        `"url":"\\/laravel-snippets\\/${escapedRoute}\\/"`
    );
    // Sidebar / body hrefs to docs pages.
    html = html.replace(
        /href="\/docs\/13\.x\/([a-z0-9-]+)"/g,
        'href="/laravel-snippets/docs/13.x/$1/"'
    );
    return html;
}
...
async function main(): Promise<void> {
    ...
    const cssHrefForGhPages = '/laravel-snippets/laravel-docs.css';
    ...
    try {
        ...
        // Inertia sweep report. Written as dist/report.html (not
        // dist/report/index.html) so GH Pages serves it at
        // /laravel-snippets/report with no trailing slash.
        await renderRoute(
            '/report',
            join(DIST_DIR, 'report.html'),
            '"url":"\\/report"',
            '"url":"\\/laravel-snippets\\/report"',
        );
